...
do 
  ...
  # train
  python -m seg_3d.train_loop -n $NAME with "CONFIG_FILE=$CFG" "LOAD_ONLY_CFG_FILE=True" "DATASET.FOLD=$COUNT"
  # eval
  echo "Evaluating on test set for fold $COUNT...";
  python -m seg_3d.train_loop -n $NAME with "CONFIG_FILE=$CFG" "LOAD_ONLY_CFG_FILE=True" "DATASET.FOLD=$COUNT" "EVAL_ONLY=True"
